...
export default function MenuSectionClient({ isDashboard = false, menuItems = [], outlets = [] }: { isDashboard?: boolean, menuItems: any[], outlets: any[] }) {
  ...
  const [selectedOutlet, setSelectedOutlet] = useState(outlets[0]?.slug || 'depok');
  ...
  const openOrderModal = (menuName: string, price: number) => {
    ...
    setSelectedOutlet(outlets[0]?.slug || 'depok');
    setBookingDate('');
    setBookingTime('');
    setShowModal(true);
  };
  ...
  const filteredItems = menuItems.filter(item => {
    ...
    if (isDashboard) return item.is_best_seller;
      ...
      <div className="menu-grid-modern">
        {filteredItems.map((item, idx) => (
          <div className="menu-card-modern menu-reveal" key={idx} style={{ transitionDelay: `${(idx % 4) * 0.1}s` }}>
            {item.is_best_seller && <div className="bestseller-badge">🔥 Best Seller</div>}
            ...
            <div className="menu-content-modern">
              ...
              <p className="menu-desc-modern">{item.description}</p>
              ...
              <div className="menu-actions-modern">
                <button className="btn-modern btn-buy-modern" onClick={() => openOrderModal(item.order_name || item.name, item.price)}>
                  Beli
                </button>
                <button className="btn-modern btn-cart-modern" onClick={() => addToCart(item.order_name || item.name, item.price, item.image)}>
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="sleek-group" style={{ display: 'flex', gap: '10px' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <select className="sleek-input" value={deliveryMethod} onChange={e => setDeliveryMethod(e.target.value)}>
